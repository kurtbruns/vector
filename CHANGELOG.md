...
## [0.13.0] - 2026-07-21
### Added
- Exported SVGs (FIGMA / ILLUSTRATOR targets, via `bundle()` and `download()`) now carry a
  `<desc>Generated by @kurtbruns/vector</desc>` provenance stamp — the analog of Hugo's
  `<meta name="generator">`. Exported SVGs carry no watermark (only rasters do), so this invisible
  marker records their origin and survives Figma / Illustrator import as a layer description. Browser
  render output is unaffected.
