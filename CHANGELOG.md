...
## [0.12.0] - 2026-07-16
### Added
- `Shape.attachArrow()`, spelled correctly. Available on anything extending `Shape`, so `Line` and
  `Path` both take it.
### Deprecated
- `Shape.attatchArrow()`, which was the original misspelling. It forwards to `attachArrow` unchanged,
  so nothing breaks and there is no rush to migrate. Rename call sites when convenient.
