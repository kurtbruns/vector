...
import { AnimationFunction, CoordinateSystem, Group, Point, TeX } from "..";
...
export class Scene3D {
    ...
    registerEventListenersOld(r: number) {

        let isDragging = false;
        let prevX: number = 0;
        let prevY: number = 0;
        let prevPosition: Vector3 = null;
        let prevOrientation: Quaternion = null;
        const bbox = this.viewPort.frame.root.getBoundingClientRect();
        this.viewPort.frame.root.style.border = '1px solid';
        let count: number = 0;
        ...
        const projectOnTrackball = (touchX: number, touchY: number) => {
            ...
            // let x = touchX / window.innerWidth * 2 - 1;
            // let y = -(touchY / window.innerHeight * 2 - 1);
            let x = (touchX - bbox.left - bbox.width / 2) / bbox.height * 2;
            let y = -(touchY - bbox.top - bbox.height / 2) / bbox.height * 2;
            let z = 0.0;
            let distance = x * x + y * y;
            if (distance <= r * r / 2) {
                // Inside sphere
        ...
        // this.drawPoint(v, 'var(--yellow)');
        // this.drawComponents(v);

        let w = new Vector3(0, 0, 0);
        // this.drawPoint(w, 'var(--purple)');
        // this.drawComponents(w);     

        let u = Quaternion.standardForwardDirection();
        // u.scale(5);
        this.drawPoint(u, {color:'var(--main)'});
        this.drawComponents(u);

        let c = Quaternion.standardForwardDirection();
        // u.scale(5);
        this.drawPoint(c, {color:'var(--yellow)'});
        ...
        const handleMouseMove = (event: MouseEvent) => {
            ...
            if (isDragging && (event.clientX !== prevX || event.clientY !== prevY)) {
                ...
                // Use the delta to calculate new rotation
                const v1 = projectOnTrackball(prevX, prevY);
                // w.x = v1.x;
                // w.y = v1.y;
                // w.z = v1.z;
                ...
                // Angle of rotation
                const angle = Math.acos(Math.min(1.0, v1.dot(v2)));

                // Create Quaternion from axis and angle
                let q = Quaternion.fromAxisAngle(axis, angle);
                // let t = Quaternion.standardForwardDirection();
                // t.applyQuaternion(q).normalize();

                const q1 = Quaternion.fromVector(v1);
                const q2 = Quaternion.fromVector(v2);

                const r = q2.multiply(q1.conjugate()).normalize();
                this.camera.position.applyQuaternion(r.inverse());
                this.camera.orientation = this.camera.orientation.multiply(r);
                ...
                // // Working but not that good implementation
                // this.camera.position.applyQuaternion(q.inverse());
                // this.camera.updateDependents();
                // this.camera.orientation = this.camera.orientation.multiply(q);
                ...
                // let p = Quaternion.fromAxisAngle(Vector3.xAxis(), Math.PI/2).rotatePoint(v2);
                // p = Quaternion.fromAxisAngle(Vector3.xAxis(), Math.PI).rotatePoint(p);
                // this.camera.lookAt(this.origin, new Vector3(0, 0, -1), p.scale(this.cameraDistance));


                // let r = Quaternion.fromAxisAngle(Vector3.xAxis(), Math.PI / 2).multiply(q)
                // this.camera.position = Quaternion.standardForwardDirection().applyQuaternion(r).scale(this.cameraDistance);
                ...
                // this.camera.position.scale(this.cameraDistance);
                ...
                // let p = Quaternion.fromAxisAngle(Vector3.xAxis(), -Math.PI/2).rotatePoint(v2);
                // // let p = Quaternion.fromDirection(new Vector3(1,1,0.5)).rotatePoint(v2);
                // p = Quaternion.fromAxisAngle(Vector3.zAxis(), -Math.PI).rotatePoint(p);
                // this.camera.lookAt(this.origin, new Vector3(0, 0, -1), p.scale(this.cameraDistance));


                // let p = Quaternion.fromAxisAngle(Vector3.yAxis(), Math.PI).rotatePoint(v2);
                // p = Quaternion.fromAxisAngle(Vector3.xAxis(), Math.PI).rotatePoint(p);
                // this.camera.lookAt(this.origin, new Vector3(0, -1, 0), p.scale(this.cameraDistance));
